...
import random
from design import draw_collision_effect, overlay_spark_image
from utils import compute_center, generate_colors, calculate_dynamic_distance_threshold
from collision import check_collision
from statevar import GlobalState
from match import match_trackers_to_detections

# Inizializza lo stato globale
state = GlobalState()

# Carica il modello YOLOs
...
out = cv2.VideoWriter(state.OUTPUT_PATH, cv2.VideoWriter_fourcc(*'mp4v'), fps, (state.w, state.h))
...
while cap.isOpened():
    ...
    state.trackers = match_trackers_to_detections(state.trackers, detections, frame, state)

    for i, tracker1 in state.trackers.items():
        for j, tracker2 in state.trackers.items():
    ...
    for pair, cooldown in list(state.explosion_cooldown.items()):
        if cooldown > 0:
            i, j = pair
            tracker1 = state.trackers.get(i)
            tracker2 = state.trackers.get(j)
    ...
    for i, tracker in state.trackers.items():
